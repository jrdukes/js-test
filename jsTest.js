...
const change = str => {
  ...
  const firstElementGrabber = element => {
    const numberOfMembers = element.split('|').length
    if (numberOfMembers > 1) {
      const randomNumber = Math.floor(Math.random() * numberOfMembers)
      console.log('num', numberOfMembers)
  ...
  const arrayOfCleanStrings = myArray.map(firstElementGrabber).join('')
